...
#
# Usage: bash preprocess-barra.sh {input_var} {input_freq} {output_var} {output_freq} {output_grid} {regrid_method} {chunking} {flags}
#
...
for year in $(seq 1979 2024); do
    infiles=(`ls ${indir}/${input_var}_*_${year}??-${year}??.nc`)
    outfile=${output_var}_${output_grid}_ERA5_historical_hres_BOM_BARRAR2_v1_${output_freq}_${year}0101-${year}1231.nc
    
    python_command="${python} preprocess.py ${infiles[@]} ${input_var} ${input_freq} ${output_var} ${output_freq} ${output_grid} ${regrid_method} ${outdir}/${outfile} --chunking_strategy ${chunking} --compute"
    if [[ "${flags}" == "-n" ]] ; then
        echo ${python_command}
    else
        echo ${year}
