...
#PBS -l wd
#PBS -v gcm,rcm,run,exp,invar,infreq,outvar,outfreq,outgrid,regrid

# Example:
#   qsub -v gcm=ACCESS-CM2,rcm=BARPA-R,run=r4i1p1f1,exp=historical,invar=tas,infreq=1hr,outvar=tas,outfreq=1hr,outgrid=AUST-20i,regrid=conservative preprocess-cordex-job.sh
#
# Compute estimates:
...
command="bash /home/599/dbi599/bias-correction-data-release/preprocess-cordex.sh ${gcm} ${rcm} ${run} ${exp} ${invar} ${infreq} ${outvar} ${outfreq} ${outgrid} ${regrid}"
echo ${command}
${command}
