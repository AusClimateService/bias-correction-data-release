...
#PBS -l wd
#PBS -v input_var,input_freq,output_var,output_freq,output_grid,regrid_method,chunking,chunksize

# Example:
#   qsub -v input_var=tas,input_freq=1hr,output_var=tas,output_freq=1hr,output_grid=AUST-20i,regrid_method=conservative,chunking=spatial,chunksize=20 preprocess-barra-job.sh
#
# Compute usage:
...
command="bash /home/599/dbi599/bias-correction-data-release/preprocess-barra.sh ${input_var} ${input_freq} ${output_var} ${output_freq} ${output_grid} ${regrid_method} ${chunking} ${chunksize}"
echo ${command}
${command}
